---
title: API Reference
description: Full reference for the kardashev Python and JavaScript/TypeScript clients — direct ISO scrapers and the hosted Kardashev Labs energy data API.
---
...
| `outages(opts)` | Generator outages (unit-level; may 500 on some filters) |
...
| `nuclearStatus(opts?)` | NRC daily reactor status (`unit` partial match, `days`) |
| `nuclearSummary()` | Nuclear fleet capacity/output summary (returns one object, not rows) |
